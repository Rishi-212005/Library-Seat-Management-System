...
// Create seat booking function
// Create seat booking function
function createSeatBooking($studentId, $seatId, $isComputer, $bookingDate, $startTime, $duration) {
    global $pdo;
    ...
    try {
        ...
        $stmt->execute([$seatId, $bookingDate, $endTime, $startTime]);
        $conflict = $stmt->fetch();
        
        if ($conflict['count'] > 0) {
            return ['success' => false, 'error' => 'Seat is already booked for the selected time slot.'];
        }
        
        // Check if student already has ANY booking for the same date (prevent multiple bookings per day)
        $stmt = $pdo->prepare("
            SELECT COUNT(*) as count FROM seat_bookings 
            WHERE student_id = ? 
            AND booking_date = ? 
            AND status IN ('booked', 'attended')
        ");
        $stmt->execute([$studentId, $bookingDate]);
        $studentConflict = $stmt->fetch();

        if ($studentConflict['count'] > 0) {
            return ['success' => false, 'error' => 'You already have a booking for this date. Only one booking per day is allowed.'];
        }
        
        // Generate booking codes
        $bookingCode = strtoupper(substr(md5(uniqid()), 0, 8));
        $attendanceCode = strtoupper(substr(md5(uniqid() . time()), 0, 12));
        
        // Insert booking
        $stmt = $pdo->prepare("
            INSERT INTO seat_bookings (
                student_id, seat_id, is_computer, booking_date, start_time, duration,
                status, booking_code, attendance_code, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, 'booked', ?, ?, NOW())
        ");
        
        $stmt->execute([
            $studentId, $seatId, $isComputer, $bookingDate, $startTime, $duration,
            $bookingCode, $attendanceCode
        ]);
...
function sendBookingConfirmationEmail($student, $seatId, $bookingDate, $startTime, $duration, $bookingCode, $attendanceCode, $status) {
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'libraryseatmanagement@gmail.com'; // Your Gmail address
        $mail->Password   = 'jgrx rble igqm alvm'; // Your Gmail password or app password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // Enable TLS encryption
        $mail->Port       = 465; // TCP port to connect to
        
        // Recipients
        $mail->setFrom('your-library-email@gmail.com', 'Library Management System');
        $mail->addAddress($student['email'], $student['student_name']); // Add student email
        
        // Content
        $mail->isHTML(true);
        $mail->Subject = 'Library Seat Booking Confirmation';
        ...
        $mail->Body = "
            ...
            <body>
                <div class='header'>
                    <h2>Library Seat Booking Confirmation</h2>
                    <p>District Library,Anantapur </p>
                </div>
                
                <div class='content'>
                    <p>Dear {$student['student_name']},</p>
                    
                    <p>Your library seat booking has been confirmed with the following details:</p>
                    
                    <div class='details'>
                        <p><strong>Seat Number:</strong> {$seatId}</p>
                        <p><strong>Booking Date:</strong> " . date('F j, Y', strtotime($bookingDate)) . "</p>
                        <p><strong>Time Slot:</strong> " . formatTime($startTime) . " - " . formatTime($startTime + $duration) . "</p>
                        <p><strong>Duration:</strong> {$duration} hour" . ($duration > 1 ? 's' : '') . "</p>
                        <p><strong>Booking Code:</strong> <span class='code'>{$bookingCode}</span></p>
                        <p><strong>Attendance Code:</strong> <span class='code'>{$attendanceCode}</span></p>
                        <p><strong>Status:</strong> " . ucfirst($status) . "</p>
                    </div>
                    
                    <p><strong>Important:</strong> Please present your attendance code to the librarian when you arrive.</p>
                    <p><strong>Please note:<strong> If you do not check in within 15 minutes of your booking start time, your seat reservation will be automatically cancelled and made available to others</p>
                    
                    <div class='footer'>
                        <p>This is an automated message. Please do not reply directly to this email.</p>
                        <p>© " . date('Y') . "District Library,Anantapur </p>
                    </div>
                </div>
            </body>
            </html>
        ...
        $mail->AltBody = "
            Library Seat Booking Confirmation\n
            -------------------------------\n
            Student: {$student['student_name']}\n
            Seat Number: {$seatId}\n
            Date: " . date('F j, Y', strtotime($bookingDate)) . "\n
            Time: " . formatTime($startTime) . " - " . formatTime($startTime + $duration) . "\n
            Duration: {$duration} hour" . ($duration > 1 ? 's' : '') . "\n
            Booking Code: {$bookingCode}\n
            Attendance Code: {$attendanceCode}\n
            Status: " . ucfirst($status) . "\n\n
            Please present your attendance code to the librarian when you arrive.\n\n
            © " . date('Y') . " District Library,Anantapur 
        ";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['book_seat'])) {
    ...
    $emailSent = sendBookingConfirmationEmail(
        ...
        $bookingResult['attendance_code'], // Added this parameter
        'booked'
    );
...
<body>
    ...
    <form id="profileForm" action="student-profile.php" method="POST">
    ...
    bookingDateInput.addEventListener('input', function () {
        ...
        if (day === 0) {
            alert("Bookings are not allowed on Sundays. Please choose another date.");
            this.value = ''; // Clear invalid selection
            updateSeatAvailability(); // Optional: reset seats
        }
    });
...
// Example: replace this with actual student ID from server
const studentId = <?php echo json_encode($_SESSION['student_id'] ?? null); ?>;

document.addEventListener('DOMContentLoaded', () => {
    if (studentId) {
        document.getElementById('studentIdInput').value = studentId;
    } else {
        console.warn('Student ID not found. My Profile button may not work properly.');
    }
});
